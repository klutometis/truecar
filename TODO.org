...
* TODO Statement
  Guido’s position on the GIL is essentially: “Use another mechanism
  (besides threads) for concurrency!” I suspect the GIL was expedient
  at some stage in Python’s development, but has become technical
  debt. Guido has some inertia (despite working half-time on Python)
  because any alternative to GIL (e.g. fine-grained locks on mutable
  data structures) must be performant and backwards compatible.

  Using =Py_BEGIN_ALLOW_THREADS= and =Py_END_ALLOW_THREADS=, it’s
  possible for extensions to bypass the GIL; typical use-cases are
  blocking I/O and long Python-agnostic computations.
  ...
  or when [[file:Modules/zlibmodule.c][Modules/zlibmodule.c]] deflates its =z_stream=:
