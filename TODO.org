...
* TODO Statement
  Guido’s position on the GIL is: “Use another mechanism for
  concurrency (other than threads)!” I suspect the GIL was expedient
  at some stage in Python’s development, but now represents technical
  debt. Guido has some inertia (despite being being paid 50% to work
  on Python) because any alternative to GIL (e.g. fine-grained locks
  on mutable data structures) must be performant and backwards
  compatible.
